"""Audit patient-specific respiratory / gas-exchange belief features.

This tests whether an online predict-update respiratory belief state improves
...
from eicu_respiratory_target_router import _fit_ridge
from eicu_respiratory_transition_extract import TARGET_VARS
from eicu_sepsis_target_router import _bootstrap_ci, _feature_columns, _round, split_subjects
from respiratory_belief import (
    RESPIRATORY_BELIEF_COLUMNS,
    placebo_respiratory_belief_features,
    respiratory_belief_state_features,
)


DEFAULT_TARGETS = TARGET_VARS
METHODS = ("baseline_ridge", "respiratory_belief_ridge", "placebo_belief_ridge")
...
def attach_belief(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    extra = respiratory_belief_state_features(frame)
...
def _fit_predictions(
    ...
    train_placebo = pd.concat([
        train,
        placebo_respiratory_belief_features(train, seed=seed + 202),
    ], axis=1)
    heldout_placebo = pd.concat([
        heldout,
        placebo_respiratory_belief_features(heldout, seed=seed + 303),
    ], axis=1)
    placebo_columns = [f"placebo_{column}" for column in RESPIRATORY_BELIEF_COLUMNS]
    ...
    return {
        ...
        "respiratory_belief_ridge": belief_pred,
...
def _target_report(
    ...
    candidate = predictions["respiratory_belief_ridge"]
    return {
...
def split_report(
    ...
    belief_columns = belief_columns or list(RESPIRATORY_BELIEF_COLUMNS)
    target_reports = {
...
def audit(
    ...
    *,
    ...
    active_column = "respiratory_active_t"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cohort", default="eicu_respiratory_transitions_6h.parquet")
    parser.add_argument("--future-suffix", default="tp6")
    parser.add_argument("--targets", default=",".join(DEFAULT_TARGETS))
    parser.add_argument("--output", default="eicu_respiratory_belief_audit.json")
...
def main() -> None:
    ...
    report = {
        "experiment": "eICU respiratory predict-update belief downstream observable audit",
        "belief": {
            "name": "respiratory_gas_exchange_patient_specific_state",
            "columns": RESPIRATORY_BELIEF_COLUMNS,
            "direct_hidden_state_accuracy_claim_allowed": False,
            "requires_capacity_matched_placebo": True,
            "targets": targets,
        },
        "gate": {
            "candidate": "ridge_realfit_plus_respiratory_belief_features",
