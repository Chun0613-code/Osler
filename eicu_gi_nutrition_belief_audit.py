"""Audit patient-specific GI / pancreatic / nutrition belief features."""
...
from gi_nutrition_belief import GI_BELIEF_COLUMNS, gi_nutrition_belief_state_features, placebo_gi_belief_features
...
METHODS = ("baseline_ridge", "gi_belief_ridge", "placebo_belief_ridge")
...
def attach_belief(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    extra = gi_nutrition_belief_state_features(frame)
...
def _fit_predictions(
    ...
    train_placebo = pd.concat([train, placebo_gi_belief_features(train, seed=seed + 202)], axis=1)
    heldout_placebo = pd.concat([heldout, placebo_gi_belief_features(heldout, seed=seed + 303)], axis=1)
    placebo_columns = [f"placebo_{column}" for column in GI_BELIEF_COLUMNS]
...
def _target_report(
    ...
    candidate = predictions["gi_belief_ridge"]
    return {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input", default="eicu_gi_pancreatic_nutrition_transitions_6h.parquet")
    parser.add_argument("--future-suffix", default="tp6")
    parser.add_argument("--targets", default=",".join(DEFAULT_TARGETS))
    parser.add_argument("--output", default="eicu_gi_nutrition_belief_audit.json")
...
def main() -> None:
    ...
    output = {
        "artifact": "GI pancreatic nutrition belief audit",
        "input": args.input,
