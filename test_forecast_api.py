...
class ForecastApiContractTests(unittest.TestCase):
    ...
    def test_target_specific_coverage_and_raw_physical_domain_are_preserved(self):
        response = forecast_response(sample_payload())
        creatinine_24h = next(
            item for item in response["forecasts"]
            if item["target"] == "creatinine"
            and item["horizon_hours"] == 24
            and item["tier"] == "validated_artifact"
        )
        urine_6h = next(
            item for item in response["forecasts"]
            if item["target"] == "urine_output"
            and item["horizon_hours"] == 6
            and item["tier"] == "validated_artifact"
        )
        self.assertEqual(creatinine_24h["interval_target_coverage"], 0.89)
        self.assertLess(urine_6h["lower"], 0.0)
        self.assertEqual(urine_6h["physical_domain"]["minimum"], 0.0)
        self.assertIsNone(urine_6h["physical_domain"]["maximum"])
        self.assertIn("may extend below physiological support", urine_6h["note"])
    ...
    def test_model_health_runs_all_twelve_inference_smokes(self):
        health = model_health_response()
        self.assertTrue(health["ready"])
        self.assertEqual(health["integrity_load"]["status"], "passed")
        self.assertEqual(health["integrity_load"]["loaded_artifacts"], 12)
        self.assertEqual(health["inference_smoke"]["status"], "passed")
        self.assertEqual(health["inference_smoke"]["validated_artifacts"], 12)
        self.assertEqual(len(health["inference_smoke"]["checks"]), 12)

    def test_model_health_degrades_when_population_predict_fails(self):
        artifact = next(iter(runtime._precision_artifacts().values()))
        with patch.object(
            artifact.population_anchor,
            "predict",
            side_effect=RuntimeError("deterministic predict failure"),
        ):
            health = model_health_response()
        self.assertFalse(health["ready"])
        self.assertEqual(health["integrity_load"]["status"], "passed")
        self.assertEqual(health["integrity_load"]["loaded_artifacts"], 12)
        self.assertEqual(health["inference_smoke"]["status"], "failed")
        self.assertIn("deterministic predict failure", health["inference_smoke"]["error"])
    ...
    def test_monitoring_case_contract_is_ordered_and_unit_complete(self):
        ...
        self.assertEqual(case["display_name"], "Demo ICU Patient 01")
