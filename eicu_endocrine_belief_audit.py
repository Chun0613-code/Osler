"""Audit patient-specific endocrine / glycemic-stress belief features.

This tests whether an online predict-update endocrine-metabolic belief state
improves downstream observable prediction beyond both:
...
from eicu_body_system_configs import get_body_system_config
...
from endocrine_belief import (
    ENDOCRINE_BELIEF_COLUMNS,
    endocrine_belief_state_features,
    placebo_endocrine_belief_features,
)


DEFAULT_TARGETS = get_body_system_config("endocrine_stress").targets
METHODS = ("baseline_ridge", "endocrine_belief_ridge", "placebo_belief_ridge")
...
def attach_belief(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    extra = endocrine_belief_state_features(frame)
...
def _fit_predictions(
    ...
    train_placebo = pd.concat([
        train,
        placebo_endocrine_belief_features(train, seed=seed + 202),
    ], axis=1)
    heldout_placebo = pd.concat([
        heldout,
        placebo_endocrine_belief_features(heldout, seed=seed + 303),
    ], axis=1)
    placebo_columns = [f"placebo_{column}" for column in ENDOCRINE_BELIEF_COLUMNS]
    ...
    return {
        ...
        "endocrine_belief_ridge": belief_pred,
...
def _target_report(
    ...
    candidate = predictions["endocrine_belief_ridge"]
    return {
...
def split_report(
    ...
    belief_columns = belief_columns or list(ENDOCRINE_BELIEF_COLUMNS)
    target_reports = {
...
def audit(
    ...
    *,
    ...
    active_column = "endocrine_stress_active_t"
    subject_column = _subject_column(frame)
    return {
        "cohort": cohort.name,
        "future_suffix": future_suffix,
        "belief_columns": belief_columns,
        "cohort_summary": {
            "rows": int(len(frame)),
            "subjects": int(frame[subject_column].nunique()) if subject_column in frame else None,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cohort", default="eicu_endocrine_stress_transitions_6h.parquet")
    parser.add_argument("--future-suffix", default="tp6")
    parser.add_argument("--targets", default=",".join(DEFAULT_TARGETS))
    parser.add_argument("--output", default="eicu_endocrine_belief_audit.json")
...
def main() -> None:
    ...
    report = {
        "experiment": "eICU endocrine predict-update belief downstream observable audit",
        "belief": {
            "name": "endocrine_glycemic_stress_patient_specific_state",
            "columns": ENDOCRINE_BELIEF_COLUMNS,
            "direct_hidden_state_accuracy_claim_allowed": False,
            "requires_capacity_matched_placebo": True,
            "targets": targets,
        },
        "gate": {
            "candidate": "ridge_realfit_plus_endocrine_belief_features",
