"""Audit patient-specific immune / inflammatory belief features.

This tests whether an online predict-update immune belief state improves
...
from immune_inflammatory_belief import (
    IMMUNE_BELIEF_COLUMNS,
    immune_inflammatory_belief_state_features,
    placebo_immune_belief_features,
)
...
METHODS = ("baseline_ridge", "immune_belief_ridge", "placebo_belief_ridge")
...
def attach_belief(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    extra = immune_inflammatory_belief_state_features(frame)
...
def _fit_predictions(
    ...
    train_placebo = pd.concat([train, placebo_immune_belief_features(train, seed=seed + 202)], axis=1)
    heldout_placebo = pd.concat([heldout, placebo_immune_belief_features(heldout, seed=seed + 303)], axis=1)
    placebo_columns = [f"placebo_{column}" for column in IMMUNE_BELIEF_COLUMNS]
    ...
    return {
        ...
        "immune_belief_ridge": belief_pred,
...
def _target_report(
    ...
    candidate = predictions["immune_belief_ridge"]
    return {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input", default="eicu_immune_inflammatory_transitions_6h.parquet")
    parser.add_argument("--future-suffix", default="tp6")
    parser.add_argument("--targets", default=",".join(DEFAULT_TARGETS))
    parser.add_argument("--output", default="eicu_immune_belief_audit.json")
...
def main() -> None:
    ...
    output = {
        "artifact": "immune inflammatory belief audit",
        "input": args.input,
