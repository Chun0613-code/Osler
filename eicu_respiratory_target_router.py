"""Nested factual target router for the full-eICU respiratory adapter."""
...
from eicu_respiratory_transition_extract import ACTION_KEYS, TARGET_VARS
...
RESPIRATORY_METHODS = ("persistence", "population_delta", "ridge_realfit")
...
def _build_base_rows(frame: pd.DataFrame, scales: dict[str, float], future_suffix: str) -> pd.DataFrame:
    parts = []
    for target in TARGET_VARS:
        ...
        parts.append(pd.DataFrame({
            ...
            "target": target,
            "active_respiratory": selected.get(
                ACTIVE_COLUMN,
...
def attach_sources(
    ...
    *,
    ...
    features_by_target = {target: _feature_columns(frame, target) for target in TARGET_VARS}
...
def _scope(rows: pd.DataFrame, active_only: bool) -> pd.DataFrame:
    return rows[rows["active_respiratory"].astype(bool)] if active_only else rows
...
def _choose_selector(
    ...
    for target_index, target in enumerate(TARGET_VARS):
        ...
        target_details = {}
        for method_index, method in enumerate(RESPIRATORY_METHODS):
        ...
        candidates = {}
        for method in RESPIRATORY_METHODS:
...
def _fixed_methods_summary(rows: pd.DataFrame, normalized: bool) -> dict[str, float | None]:
    return {method: _round(_mae(rows, method, normalized=normalized)) for method in RESPIRATORY_METHODS}
...
def _split_report(
    ...
    return {
        ...
        "heldout_all_windows": _scope_summary(heldout, selector, active_only=False, seed=seed + 20_000, samples=bootstrap_samples),
        "heldout_active_respiratory_only": _scope_summary(heldout, selector, active_only=True, seed=seed + 30_000, samples=bootstrap_samples),
    }
...
def _multi_seed_summary(reports: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "target_router": {
            "active_respiratory_only": collect("heldout_active_respiratory_only"),
...
def _cohort_summary(frame: pd.DataFrame, future_suffix: str = DEFAULT_FUTURE_SUFFIX) -> dict[str, object]:
    summary = {
        ...
        "active_respiratory_rows": int(frame[ACTIVE_COLUMN].fillna(False).sum()) if ACTIVE_COLUMN in frame else None,
    }
    for target in TARGET_VARS:
        current = f"{target}_t"
        future = _future_column(target, future_suffix)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="eicu_respiratory_target_router.json")
...
def main() -> None:
    ...
    report = {
        "experiment": "Full eICU respiratory nested factual target router",
        ...
        "selector_gate": {
            "scope": "discovery active-respiratory rows",
