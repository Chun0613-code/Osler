"""Audit patient-specific cardiovascular belief features.

This tests whether an online predict-update cardiovascular/perfusion belief
state improves downstream observable prediction inside the eICU cardiovascular
instability module.  The gate mirrors the renal belief discipline:

* baseline = factual ridge model using the usual observed variables;
* candidate = baseline + cardiovascular belief features;
* placebo = baseline + same number of random-noise features;
* validation requires held-out improvement over both baseline and placebo.

The output is aggregate-only and does not permit causal, counterfactual,
clinical, runtime, checkpoint, or active-rule authority.
"""
...
from cardiovascular_belief import (
    CARDIOVASCULAR_BELIEF_COLUMNS,
    cardiovascular_belief_state_features,
    placebo_cardiovascular_belief_features,
)
...
DEFAULT_TARGETS = ("map", "heart_rate", "lactate", "o2sat", "respiratory_rate")
METHODS = ("baseline_ridge", "cardiovascular_belief_ridge", "placebo_belief_ridge")
...
def attach_belief(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    extra = cardiovascular_belief_state_features(frame)
...
def _fit_predictions(
    ...
    train_placebo = pd.concat([
        train,
        placebo_cardiovascular_belief_features(train, seed=seed + 202),
    ], axis=1)
    heldout_placebo = pd.concat([
        heldout,
        placebo_cardiovascular_belief_features(heldout, seed=seed + 303),
    ], axis=1)
    placebo_columns = [f"placebo_{column}" for column in CARDIOVASCULAR_BELIEF_COLUMNS]
    ...
    return {
        ...
        "cardiovascular_belief_ridge": belief_pred,
...
def _target_report(
    ...
    candidate = predictions["cardiovascular_belief_ridge"]
    return {
...
def split_report(
    ...
    belief_columns = belief_columns or list(CARDIOVASCULAR_BELIEF_COLUMNS)
    target_reports = {
    ...
    *,
    ...
    return {
        ...
        "cohort_summary": {
            ...
            "active_rows": int(frame["cardiovascular_instability_active_t"].fillna(False).sum()) if "cardiovascular_instability_active_t" in frame else None,
        },
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cohort", default="eicu_cardiovascular_instability_transitions_6h.parquet")
    parser.add_argument("--future-suffix", default="tp6")
    parser.add_argument("--targets", default=",".join(DEFAULT_TARGETS))
    parser.add_argument("--output", default="eicu_cardiovascular_belief_audit.json")
...
def main() -> None:
    ...
    report = {
        "experiment": "eICU cardiovascular predict-update belief downstream observable audit",
        "belief": {
            "name": "cardiovascular_perfusion_shock_burden",
            "columns": CARDIOVASCULAR_BELIEF_COLUMNS,
            "direct_hidden_state_accuracy_claim_allowed": False,
            "requires_capacity_matched_placebo": True,
            "targets": targets,
        },
        "gate": {
            "candidate": "ridge_realfit_plus_cardiovascular_belief_features",
